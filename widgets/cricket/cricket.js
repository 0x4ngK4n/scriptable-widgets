// Functions and variables used by Scriptable.
// These must be at the very top of the file. Do not edit.
// Widget to track cricket match score.

// I am at the mercy of below URL and it's JSON data's structure at the time of writing this code.
// If in future the JSON data changes, the script would need to be adjusted.
const url = 'https://hs-consumer-api.espncricinfo.com/v1/pages/matches/current?latest=true'
const req = new Request(url)
const res = await req.loadJSON()
y = res.matches

//these were the teams' I was interested. Feel free to add your nation if cricket is played.
//P.S. If you do add an entry, delete some other entrie(s) from the below list as widgets do not have a large display area.
...
for (i=0; i < y.length; i++) {
       //show only international matches
      if(typeof(y[i].internationalClassId) == "number") {
        if (y[i].liveInning == null) {
          continue
        }
        //show only the on-going matches
        if (!checkDate(y[i].startDate, y[i].endDate)) {
          continue
        }
        //show only matches from the team's in the aforementioned array
